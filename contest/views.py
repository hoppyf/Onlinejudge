...
def contest_list_page(request, page=1):
    ...
    current_page, previous_page, next_page, page_range, error, one_page = do_pages(contests, 10, page)
    if error:
        return error_page(request, u"不存在的页码")
    ...
    return render(request, "oj/contest/contest_list.html",
                  {"contests": current_page, "page": int(page),
                   ...
                   "keyword": keyword, "join": join, "page_range": page_range,
                   "one_page": one_page, "start_id": page * 10 - 10})
