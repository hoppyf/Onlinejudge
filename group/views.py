    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if group.join_group_setting == 0:
                ...
                if not data["message"]:
                    return error_response(u"申请信息是必填项。")
                try:
...
def group_list_page(request, page=1):
    ...
    if join:
        groups = groups.filter(join_group_setting=int(join))

    # 写的巨臭的状态筛选，等待更新
    for group in groups:
        try:
            AdminGroupRelation.objects.get(group=group, user=request.user)
            group.is_joined = 2
        except:
            try:
                UserGroupRelation.objects.get(group=group, user=request.user)
                group.is_joined = 1
            except:
                if JoinGroupRequest.objects.filter(group=group, user=request.user):
                    group.is_joined = 3
                else:
                    group.is_joined = 0
    ...
    if status:
        for group in groups:
            if group.is_joined != int(status) - 1:
    ...
    current_page, previous_page, next_page, page_range, error, one_page = do_pages(groups, 20, page)
    if error:
        return error_page(request, u"不存在的页码")
    ...
    return render(request, "oj/group/group_list.html", {
        ...
        "keyword": keyword, "page_range": page_range, "one_page": one_page,
