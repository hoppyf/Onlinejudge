...
from utils.do_pages import do_pages
...
from .models import Problem, ProblemTag, ProblemDiscussion
        ...
        for i in range(1, test_case_number + 1):
            ...
            file_info["test_cases"][str(i)] = {"input_name": str(i) + ".in",
                                               ...
                                               "input_size": os.path.getsize(
                                                       os.path.join(test_case_dir, str(i) + ".in")),
    ...
    def get(self, request):
        ...
        try:
            ...
            for filename in os.listdir(test_case_dir):
                # 避免存在文件链接,导致真实文件被打包
                if self._is_legal_test_case_file_name(filename) and not os.path.islink(
                        os.path.join(test_case_dir, filename)):
...
def problem_list_page(request, page=1):
    ...
    current_page, previous_page, next_page, page_range, error, one_page = do_pages(problems, 40, page)
    if error:
        return error_page(request, u"不存在的页码")
...
def discussion_page(request, problem_id):
    ...
    try:
        ...
        return error_page(request, u"题目不存在")
    problem_discussion_list = ProblemDiscussion.objects.filter(problem=problem).order_by("-created_time")
    ...
    current_page, previous_page, next_page, page_range, error, one_page = do_pages(problem_discussion_list, 10, page)
    if error:
        return error_page(request, u"不存在的页码")

    return render(request, "oj/problem/problem_discussion_list.html",
                  {"problem_discussion_list": current_page, "problem": problem,
                   "previous_page": previous_page, "next_page": next_page, "page_range": page_range,
                   "one_page": one_page})


class ProblemDiscussionAPIView(APIView):
    @login_required
    def post(self, request):
        """
        post讨论内容
        ...
        try:
            ...
            return error_response(u"题目不存在")
        ProblemDiscussion.objects.create(message=msg, user=request.user, problem=problem)
